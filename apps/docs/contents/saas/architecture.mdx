...
- **Custom domains**: Pro tenants can bind one branded hostname with managed HTTPS. `TenantDomainService` enforces the plan quota, registers the hostname through Cloudflare for SaaS, persists hostname and SSL provisioning states, and activates the domain only when both Cloudflare states are `active`.
