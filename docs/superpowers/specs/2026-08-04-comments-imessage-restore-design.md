...
3. The overlay exists at the full composer geometry immediately, so clearing the real field does not create an empty intermediate frame. A target is eligible only after scroll-to-bottom is no longer pending, its frame is fully above the composer, and it remains unchanged through a short layout-settling window; this prevents an off-screen pre-scroll row frame from being locked intermittently. Once the settled `target` exists, a linear 0.4s master clock reproduces the classic ChatKit type-0 choreography without private API use: X position uses cubic `(0.54195, 0, 0.58, 1)`, Y uses `(0.49, 0.0884, 0.4055, 0.9055)`, and bounds use `(0.542, 0, 0.58, 1)`, each at 2× channel speed. Bounds morph around the trailing-center anchor; bubble scale follows `1 → 0.88 → 1` at key times `[0, 0.35, 1]`; bubble opacity reaches 1 in the first third; the white-text mask fades over 0.3s; and the send-button snapshot scales to zero over 0.25s. Destinations wider than 1.66× the source use a 0.35s master duration. The source and destination remain pixel-aligned with the complete composer pill and real bubble respectively.
...
The rect derivation, cubic-bezier inversion, right-edge anchoring, and channel choreography live in a pure `CommentFlightMath` enum so the Swift test target covers them. The constants are behaviorally derived from static disassembly of the arm64 `ChatKit.framework` in the iOS 26.5 Simulator runtime; no ChatKit symbol or private API is linked or invoked. The type-1 Liquid Glass spring path remains out of scope because this composer uses a solid system-fill surface rather than ChatKit's glass input treatment.
